...
app.get('/api/convert/:id', authenticateJWT, async (req, res) => {
  try {
    const obj = { id: req.params.id };
    res.writeHead(200, {
    ...
    await downloaders.youtube.downloadAndConvert(video.video, (state, progress) => {
      res.write(`${state} ${Math.round(progress*100)}\n`);
    });
    res.end();
  } catch(error) {
    console.error(error);
